...
The overlay auto-starts on login. Its SQLite DB at `backend/api/data/slaythelist.db`
...
1. Write the override to `frontend/web/.env.local` (Next re-reads it on restarts):
...
cd backend/api && PORT=8799 SLAYTHELIST_DATA_DIR=<scratch>/data npx tsx src/server.ts

# Web on 3005 (after writing .env.local as above)
cd frontend/web && npx next dev -p 3005
```

`npm run typecheck` at the root builds contracts + combat-engine first — run it once
before starting if `shared/contracts` changed, since both servers import the built types.
